...
function DHL (options) {
  ...
  function dimensionalWeight (parcel) {
    ...
    // DHL can't handle more than 2 digits for this -_-
    return (width * height * depth / volumentricDivisor).toFixed(2);
  }
  ...
  function buildShipXML (data) {
    ...
    xml = concatKeyValuePair(xml, 'ShipmentDetails', {
      ...
      GlobalProductCode: data.productCodes.globalProductCode,
      LocalProductCode:  data.productCodes.localProductCode,
      Date:              moment().format(xmlDateFormat),
