...
function DHL (options) {
  ...
  function buildRatesXML (data) {
    ...
    //  return acc + parseFloat(curr.unitPrice) * parseInt(curr.quantity);
    //}, 0));
  ...
  function buildShipXML (data) {
    ...
    if (typeof data.receiver.address === 'array') {
      data.receiver.address.each(function (line) {
        consignee.concatKeyValuePair('AddressLine', line);
      });
    } else {
      consignee.concatKeyValuePair('AddressLine', data.receiver.address);
    }
    consignee.push({
